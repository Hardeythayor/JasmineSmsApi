...
class ListenSKPushbulletStream extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'skpushbullet:listen';
     ...
     * @var string
     */
    protected $description = 'Connects to SK Pushbullet WebSocket stream and listens for messages.';
    ...
    {
        $apiKey = config("pushbullet.keys.sk");//$this->argument('apiKey');
        ...
        if (isset($data['type'])) {
            ...
            if ($data['type'] === 'tickle' && isset($data['subtype'])) {
                ...
                if($push['type'] === 'sms_changed') {
                    ...
                    $recipient = ThirdPartyNumber::where('label', 'skt')->first()?->phone;
